    ...
    public List<PoloniexHistoryTrade> loadTradesHistory(CurrencyPair currencyPair, Duration historyDuration) {
        ...
        while (from.compareTo(now) < 0) {
            ...
            try {
                ...
                ResponseEntity<String> response = restTemplate.exchange(properties.getApi().getTradeHistoryUrl(),
                        HttpMethod.GET, null, String.class, parameters);
                csvFileWriter.write(response.getBody(), "analytics/poloniex.json", true);
